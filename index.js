			...
			.then(async ({ data }) => {
				...
				if (new Date().getMinutes() % 10 === 0) {
					await new Promise((res, rej) => {
						...
						axios
							.get(
								`https://api.telegram.org/bot<BOT_API_KEY>/sendMessage?chat_id=<CHAT_ID>&text=Test Ping`
							)
							.then(({ data }) => {
								console.log(data);
								res(data);
				...
				if (out.length > 0) {
					const _data = await new Promise((res, rej) => {
						...
						axios
							.get(
								`https://api.telegram.org/bot<BOT_API_KEY>/sendMessage?chat_id=<CHAT_ID>&text=${out.length} slots available in Nasik for dose 1. Checked on ${_date}. Showing for ${dates}`
							)
							.then(({ data }) => {
								console.log(data);
								res(data);
